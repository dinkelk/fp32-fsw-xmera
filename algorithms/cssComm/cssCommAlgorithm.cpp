...
std::array<double, kMaxNumCssSensors> CssCommAlgorithm::update(
    ...
    for (uint32_t i = 0; i < kMaxNumCssSensors; ++i) {
        ...
        /* The raw reading only approximates a cosine; the Chebyshev correction makes it a true cosine,
           which cannot leave [0, 1]. A value that is not finite carries no measurement at all, so
           report no signal rather than pass it on. */
