 ...
 * An instance can only exist if: every maximum thrust is finite and positive; the ON/OFF duty cycle thresholds
 * are finite with 0 < levelOn <= 1,
   ...
   public:
    static ThrFiringSchmittConfig create(const ThrFiringSchmittThrusterArray& thrusterArray,
        ...
        if (!isValidThrusterArray(thrusterArray)) {
            FSW_THROW_INVALID_ARGUMENT("thrFiringSchmitt: each thruster's maxThrust must be finite and > 0.");
        }
