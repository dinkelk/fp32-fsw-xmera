...
Two bounds keep the step usable.

**A shift direction must exist.** The null space of the selected rows has dimension :math:`N - r`, with
:math:`r` the number of kept singular values. With :math:`r = N` there is no null space, and the projection
above returns only its own round-off, whose direction is arbitrary. The module tests :math:`r < N` rather than
the size of that residual. Even with a null space, :math:`\mathbf{1}` can be orthogonal to it and give
:math:`\mathbf{n} = \mathbf{0}`: a pair of parallel thrusters does this. The module thus also requires
:math:`\|\mathbf{n}\|_\infty` to be above a small floor before it shifts.

**The shift must reach the entry it lifts.** The step that brings entry :math:`j` to zero is
:math:`-F_{pre,j} / n_{j}`, so a small :math:`n_{j}` makes it large, and the shift scales every other thruster
by :math:`\alpha n_{k}`. To lift one barely-reachable thruster thus costs a large thrust on all the others.
The module therefore takes the step only from the entries with :math:`n_{j}` above a tenth of
:math:`\|\mathbf{n}\|_\infty`, and leaves the rest to the final :math:`\max(\cdot, \mathbf{0})`. A balanced
layout is unaffected: there :math:`\mathbf{n} = \mathbf{1}`, so every entry sits at the maximum.
