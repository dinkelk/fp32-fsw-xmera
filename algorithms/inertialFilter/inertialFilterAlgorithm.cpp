 ...
 *  @param config [-] validated InertialFilterConfig */
InertialFilterAlgorithm::InertialFilterAlgorithm(const InertialFilterConfig& config) : cfg(config) {
 ...
 *  @param config [-] validated InertialFilterConfig */
void InertialFilterAlgorithm::setConfig(InertialFilterConfig const& config) {
    this->cfg = config;
 ...
 *  @return void */
void InertialFilterAlgorithm::reInitializeExceptPersistentStates() {
...
InertialFilterOutput InertialFilterAlgorithm::update(double currentSeconds,
    ...
    this->srukf.setState(regularize(this->srukf.getState()));
    this->srukf.setStateLastMeasurement(regularize(this->srukf.getStateAtLastMeasurement()));

    return InertialFilterOutput{
...
bool InertialFilterAlgorithm::timeUpdate(double dt) { return this->srukf.timeUpdate(dt); }
...
bool InertialFilterAlgorithm::measurementUpdate(Measurement const& measurement) {
...
void InertialFilterAlgorithm::clear() {
    this->srukf.clear();
    this->lastStAttResiduals.valid = false;
    this->lastRateResiduals.valid = false;
}
...
StAttMeasurement InertialFilterAlgorithm::packStAttMeasurement(StAttData const& stAttData) const {
...
InertialFilterAlgorithm::State InertialFilterAlgorithm::regularize(State const& state) {
...
FilterStateOutput InertialFilterAlgorithm::getFilterOutput() const {
...
StAttResidualsOutput const& InertialFilterAlgorithm::getLastStAttResiduals() const { return this->lastStAttResiduals; }
/*! @return const reference to the latest rate residuals snapshot */
RateResidualsOutput const& InertialFilterAlgorithm::getLastRateResiduals() const { return this->lastRateResiduals; }

/*! @return current filter state (post-sanitization) */
InertialFilterAlgorithm::State InertialFilterAlgorithm::getState() const { return this->srukf.getState(); }
/*! @return current full covariance */
Eigen::Matrix<double, InertialFilterAlgorithm::N, InertialFilterAlgorithm::N> InertialFilterAlgorithm::getCovariance()
    const {
